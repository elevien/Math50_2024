...
\item We can characterize a random variable using  a \dfn{probability model}\index{probability model} or \dfn{probability distribution}\index{probability distribution}, which maps a set of possible events to real numbers between 0 and 1. 
\item  The \dfn{Bernoulli distribution} is probably the simplest probability distribution. It models a variable with binary outcome, for example the result of a YES/NO survey or a COVID test. 
